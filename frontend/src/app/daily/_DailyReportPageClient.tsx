...
export default function DailyReportPageClient() {
  ...
  const fetchReport = useCallback(async (date?: string) => {
    try {
      ...
      const data = await queryClient.fetchQuery({
        queryKey: [...queryKeys.daily.report(date || null), reportScope],
        queryFn: async () => (date
      ...
      if (topPicks && topPicks.length > 0) {
        ...
        await Promise.all(
          topPicks
            ...
            .map(async (key) => {
              try {
                const sp = await queryClient.fetchQuery({
                  queryKey: [...queryKeys.daily.all, 'sparkline', key, 48, 2],
                  queryFn: () => dailyReportApi.sparkline(key, 48, 2),
                  staleTime: 5 * 60_000,
                });
  ...
  const generateForDate = useCallback(async (date: string) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.daily.all });
      // 轮询直到 DONE（最多 120 秒）
  ...
  }, [queryClient, refreshReportIndexes]);
